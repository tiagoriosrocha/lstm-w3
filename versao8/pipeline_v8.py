...
# Nesta versao, continuamos no problema de classificacao por serie.
# Cada arquivo Parquet e tratado como uma amostra completa com uma unica classe.
# A diferenca central em relacao a versao6 esta na arquitetura recorrente:
# aqui propomos uma LSTM mais profunda e com uma cabeca de classificacao
# mais rica, buscando reduzir a distancia para a baseline tabular.
CANDIDATE_FEATURE_COLUMNS = BASE_TARGET_COLUMNS + STATE_COLUMNS + AUX_ANALOG_COLUMNS
ARTIFACTS_ROOT = PROJECT_ROOT / "artifacts" / "reports_v8"
SERIES_STAT_NAMES = [
    "mean",
    "std",
    "min",
...
def require_classification_stack() -> None:
    ...
    if (
        ...
        raise ImportError(
            ...
            "Instale scikit-learn para executar a versao8."
        )
...
def prepare_classification_artifacts(
    *,
    ...
    bundle_path = run_dir / "bundle_v8.json"
...
if torch is not None:
    class LSTMSeriesClassifier(nn.Module):
        def __init__(
            ...
            # A normalizacao ajuda a estabilizar a entrada antes da LSTM.
            ...
            pooled_size = hidden_size * self.output_multiplier

            # Em vez de usar apenas o ultimo estado oculto, combinamos tres
            # resumos complementares da sequencia:
            # 1) ultimo estado,
            # 2) media temporal,
            # 3) pooling por atencao.
            # Isso aumenta a capacidade do classificador de capturar tanto
            # padroes globais quanto eventos mais localizados.
            self.sequence_norm = nn.LayerNorm(pooled_size)
            self.attention_score = nn.Linear(pooled_size, 1)
            self.classifier = nn.Sequential(
                nn.Linear(pooled_size * 3, 256),
                nn.GELU(),
                nn.Dropout(dropout),
                nn.Linear(256, 128),
                nn.GELU(),
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            sequence_output, (hidden_state, _) = self.lstm(normalized)
            sequence_output = self.sequence_norm(sequence_output)

            # Se a LSTM for bidirecional, o PyTorch devolve os estados das
            # duas direcoes em posicoes separadas. Aqui concatenamos ambos.
            ...
            # O pooling por atencao aprende a atribuir maior peso aos instantes
            # mais relevantes para a classe final.
            ...
            features = torch.cat([last_hidden, mean_pool, attention_pool], dim=1)
            return self.classifier(features)
else:
    class LSTMSeriesClassifier:
...
def _predict_lstm_logits(
    ...
    *,
    ...
    loader = _tensor_loader(X, np.zeros(len(X), dtype=np.int64), batch_size=batch_size, shuffle=False)
    logits_parts = []
    model.eval()
    with torch.no_grad():
        for batch_x, _ in loader:
            batch_logits = model(batch_x.to(device_obj, non_blocking=True))
            logits_parts.append(batch_logits.detach().cpu().numpy())
    return np.concatenate(logits_parts, axis=0)


def predict_lstm_classes(
    model: nn.Module,
    X: np.ndarray,
    *,
    batch_size: int = 128,
    ...
    logits = _predict_lstm_logits(model, X, batch_size=batch_size, device=device)
...
def train_lstm_classifier(
    ...
    *,
    ...
    checkpoint_path = output_dir / "lstm_profunda_multiclasse_best.pt"
    config_path = output_dir / "lstm_profunda_multiclasse_config.json"
    history_path = output_dir / "lstm_profunda_multiclasse_history.csv"
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_pred = predict_lstm_classes(model, X_train, batch_size=batch_size, device=str(device_obj))
        val_pred = predict_lstm_classes(model, X_val, batch_size=batch_size, device=str(device_obj))
    ...
    return LSTMTrainingSummary(
        model_name="lstm_profunda_multiclasse",
        checkpoint_path=str(checkpoint_path),
